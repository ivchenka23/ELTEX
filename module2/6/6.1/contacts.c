...
void addContact(void) {
    ...
    ContactNode *curr = head;
    while (curr && (strcmp(curr->data.lastName, newNode->data.lastName) < 0 || 
                   (strcmp(curr->data.lastName, newNode->data.lastName) == 0 && 
                    strcmp(curr->data.firstName, newNode->data.firstName) < 0))) {
        curr = curr->next;
    }
    newNode->next = curr;
